...
def get_current_ipv6():
    """
    获取本机的全球单播IPv6地址（优先获取永久地址）
    返回: 字符串形式的IP地址，如果未找到则返回None
    """
    # 优先使用netifaces库获取更准确的地址信息
    try:
        import netifaces
            ...
            if netifaces.AF_INET6 in addrs:
                for addr_info in addrs[netifaces.AF_INET6]:
                    ...
                    if addr.startswith(('2001:', '240', '2a', '2b', '2c')):
                        # 检查是否为临时地址（通常flags中包含临时地址标记）
                        if addr_info.get('flags', 0) & 0x10:  # IFA_F_TEMPORARY = 0x10
                            temporary_addrs.append(addr)
                        else:
                            permanent_addrs.append(addr)
        
        # 优先返回永久地址，如果没有永久地址则返回临时地址
        if permanent_addrs:
            return permanent_addrs[0]
        elif temporary_addrs:
            return temporary_addrs[0]
        return None
    except ImportError:
        print("未找到netifaces库，使用socket方法。请执行 'pip install netifaces' 安装以获得更准确的结果。")
    
    # 如果netifaces不可用，使用socket方法作为备选
    try:
