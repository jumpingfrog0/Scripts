...
new_name_pre = 'aset_'
new_name_suffix = '_img'

replace_split = '   ======   '

G_ENABLE_CHANGE_HASH = False
G_NEED_CLEAN_EMPTY_IMAGESET = False

# 主包图片资源xcassets
main_assets = 'Assets.xcassets'

# 马甲包图片资源xcassets
shell_assets = 'Pyroo.xcassets'


# 忽略的图片资源名称
g_ignore_image_names = ['AppIcon', 'LaunchImage', 'Pods', shell_assets]
...
def camel_to_snake(method_name):
    """驼峰命名转下划线命名（带前缀pyroo_，适配OC方法参数分隔符:）"""
    
    snake_name = re.sub(r'(?<!^)(?=[A-Z0-9])', '_', method_name).lower()
...
def findAssetPath(curPath):
    for dir in os.listdir(curPath):
        if dir in g_ignore_image_names:
            continue
    ...
    for path in all_files:
        for file in os.listdir(path):
            if file.endswith('png'):
                ...
                print(os.popen('find '+ '\"'+ path + '\"'+' -iname \"'+ file +'\" -exec echo {} \; -exec magick {} -resize 90%  -resize 115% {} \;').read())
...
def renameImage():
    ...
    if G_ENABLE_CHANGE_HASH:
        changeHash()

    if G_NEED_CLEAN_EMPTY_IMAGESET:
        cleanEmtpyNewPreDir()
    else:
        createMap()
        replaceImageName()
