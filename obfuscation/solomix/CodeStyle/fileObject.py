...
defaultExcludeSrcDir = ["Pods","Scripts","proto","UnUsedFiles",".gitignore",".git",".DS_Store","Solomix.xcworkspace"]
    ...
    if isDir:
        ...
        for file in os.listdir(rootPath):
            ...
            if '.framework' in file:
                continue
            filePath = os.path.join(rootPath, file)
            recursiveGetProjNameAndProjDir(f, excludeDir, filePath)

# 查询项目名和根目录
def findAllProjDir(excludeDir):
    curPath = os.getcwd()
    ...
    allProjDirPaths.clear()
    recursiveGetProjNameAndProjDir(f, exclude, curPath)

# 排除掉excludeDir目录名(数组)符合targetTypes(数组)类型的所有文件目录，传空数组则用默认定义
def allSrcFilePath_2(excludeDir, targetTypes):
    allSrcFilePaths.clear()
    findAllProjDir(excludeDir)
    # rootDir = allProjDirPaths[0]
    for rootDir in allProjDirPaths:
        print(rootDir)
        recursiveSrcFilePath(rootDir, excludeDir, targetTypes)
    return allSrcFilePaths 